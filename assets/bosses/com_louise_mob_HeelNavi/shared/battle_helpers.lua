...
---@param column any
---@return Tile | nil
function battle_helpers.choose_random_tile(character, column)
    ...
    if #tile_array == 0 then return nil end
...
function battle_helpers.move_to_back_col(character)
    ...
    local target_tile = tile_array[math.random(1, #tile_array)]
    if target_tile then
        moved = character:teleport(target_tile, ActionOrder.Voluntary)
    end
    return moved
end

function battle_helpers.move_to_front_col(character)
